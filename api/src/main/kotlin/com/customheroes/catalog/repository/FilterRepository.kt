...
interface FilterRepository : CrudRepository<Filter?, Int?> {
    ...
    @Query(value = "SELECT DISTINCT filter_id, filter_table.figure_id, filter_table.tag_id FROM filter_table JOIN figure_table ON filter_table.figure_id = figure_table.figure_id WHERE tag_id=:tag_id AND LOWER(figure_table.figure_name) LIKE CONCAT('%', LOWER(:figure_title),'%')", nativeQuery = true)
